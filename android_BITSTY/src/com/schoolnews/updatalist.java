...
public class updatalist {
	...
	{ 
		...
		schoolnewsFeed feed = new schoolnewsFeed();
		...
		for (int t = 1; t <= Integer.parseInt(newweb.get("count")); t++) {
			...
			flag--;
			try {
				filessdf = new WriteXmlFile(mcontext, feed.getAllItem(),(String)oldweb.get("FileName")+flag, 2);
			} catch (IOException e) {
	 ...
	 private schoolnewsFeed getFeed(String urlString ,int old) {
	    	try {
	    		...
	    		schoolnewsHandler rssHandler = new schoolnewsHandler(old);
